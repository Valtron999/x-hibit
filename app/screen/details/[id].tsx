...
const Details = () => {
  ...
  // ─── Real data (all hooks — called unconditionally, before any early return) ──
  ...
  const moreByArtist = authorPosts.filter((p) => p.id !== post?.id);
  ...
  const { posts: gridPosts } = useAllPosts({ excludeId: post?.id });
  ...
  const loading = postLoading || authorLoading;
  ...
  // unconditionally on every render. Early returns only happen now. ──
  ...
  const Header = () => (
    <>
      ...
      <View style={{ width: "100%", marginTop: -100 }}>
        ...
        >
          <TouchableOpacity
            onPress={() => router.push(`/screen/users/${author.id}`)}
            style={{
