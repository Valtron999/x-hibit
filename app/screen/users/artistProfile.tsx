...
const Profile = () => {
    ...
    useEffect(() => {
        if (!loading && !session) {
            router.replace("../authscreen/login"); // adjust to your actual login route
        }
    }, [loading, session]);
    ...
    //  user posts — still pulling from mock data for now, filtered by the real
    //  logged-in user's id. This will read as empty until the real posts table
    //  exists, since mock ids ("u1") won't match a real Supabase UUID.
    const userPosts = postsData.filter(
        (post) => post.userId === currentUser.id
    );

    //  saved posts — same caveat, plus currentUser.savedPosts is currently
    //  always [] until the saved_posts table is built.
